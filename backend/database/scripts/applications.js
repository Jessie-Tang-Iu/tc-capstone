...
if (secretKey.length != 32) {
  throw new Error(
    "ENCRYPTION_SECRET must be 32 bytes (256 bits) long after Base64 decoding."
  );
}
...
export async function getApplicationById(id) {
  ...
  if (app.cover_letter_data && app.cover_letter_iv) {
    app.cover_letter_data = decryptData(
...
export async function updateApplicationStatus(id, status) {
  const { rows } = await query(
    `
    UPDATE application SET status = $1, applied_at = NOW() WHERE id = $2 RETURNING *`,
    [status, id]
  );
  return rows[0];
}

export async function getAllApplications(employerId) {
  const { rows } = await query(
    `
    SELECT 
      ap.id, 
