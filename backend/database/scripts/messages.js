...
export async function markDelivered(id) {
  await query(`UPDATE message SET status = 'D' WHERE id = $1`, [id]);
}

export async function markReadById(id) {
  await query(`UPDATE message SET status = 'R' WHERE id = $1`, [id]);
}
...
export async function deleteConversation(conversationId) {
  const result = await query(
    `DELETE FROM message WHERE conversation_id = $1 RETURNING id`,
    [conversationId]
  );
  return result.rows;
}
