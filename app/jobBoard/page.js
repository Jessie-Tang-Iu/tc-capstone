...
export default function JobBoardPage() {
  ...
  useEffect(() => {
    fetch("/api/job")
      ...
      .then((data) => {
        // Determine the actual array source: assume it's data itself,
        // or access a property like 'jobs' if the API wraps it.
        const jobsArray = Array.isArray(data) ? data : data.jobs;

        if (!Array.isArray(jobsArray)) {
          // Log error if jobsArray is still not an array
          console.error("API response data structure invalid:", data);
          return;
        }

        let activeJobs = jobsArray.filter((job) => job.status === "A");
        setJobs(activeJobs);
      })
  ...
  const search = useCallback(() => {
    ...
    results = results.filter((job) => {
      ...
      const matchesSalary =
        !minSalary ||
        (minSalary <= job.salary_per_hour &&
          job.salary_per_hour <= minSalary + 5);
                ...
                <div className="px-2 py-1 space-y-2">
                  {filteredJobs.map((job) => (
                    <JobCard
                      ...
                      tags={[
                        `${job.salary_per_hour} per an hour`,
                        job.type,
                        job.workplace,
                        job.experience,
                        job.industry,
